...
app.put("/cards/:id", async (req, res) => {
  try {
    ...
    const results = await client.query(
      "UPDATE business_cards SET name = $1, phone_number = $2, email = $3, occupation = $4, background_color = $5, text_color = $6,username = $7 WHERE card_id = $8",
      [
        name,
        phone_number,
        email,
...
app.put("/users/:user", async (req, res) => {
  try {
    ...
    const { username, passwords } = req.body;
    const results = await client.query(
      "UPDATE users SET passwords = $1 WHERE username = $2",
      [passwords, username]
    );
    if (!results.rows) {
